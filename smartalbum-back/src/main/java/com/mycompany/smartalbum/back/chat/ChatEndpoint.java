...
import com.mycompany.smartalbum.back.chat.coders.MessageDecoder;
import com.mycompany.smartalbum.back.chat.coders.MessageEncoder;
import com.mycompany.smartalbum.back.chat.messages.ChatMessage;
import com.mycompany.smartalbum.back.chat.messages.ChatMessagesMessage;
import com.mycompany.smartalbum.back.chat.messages.GetUsersMessage;
import com.mycompany.smartalbum.back.chat.messages.JoinMessage;
import com.mycompany.smartalbum.back.chat.messages.Message;
import com.mycompany.smartalbum.back.chat.messages.UserListMessage;
...
public class ChatEndpoint {
    ...
    public void onMessage(ByteBuffer byteBuffer, boolean complete, @PathParam("room-name") String roomName) {
        try {
            ...
            if (complete) {
                ...
                try {
                    fos = new FileOutputStream("/Users/aristidemvou/tmp/test-websocket.jpg");
    ...
    private void processMessage(JoinMessage message, String roomName) {
        ...
        if(!exists(message.getName(),usersList)){
        usersList.add(user);}
        broadcast(message, roomName);
    }
    
    private boolean exists(final String name, final List<User> usersList)
    {
