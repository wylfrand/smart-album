...
function changeCoveringImage()
{
	   var actionValue = $('select#selectedCoverAlbum option:selected').val();
	   var coveringImgUrl = "<c:url value='/controller/getByPageAndPosition/"+actionValue+".html'/>";
...
$( "#valider_infos_album" ).click(function( event ) {
	...
	<div id='container'>
		<div id='logo' class="albumhead">
			<h1>
				Creéation d'un album <span id="imageSize"/></span>
			</h1>
		</div>
		<div id='content'>
			<!-- modal content -->
			<form:form modelAttribute="albumForm" id="albumId" name="albumId" method="post" action="${pageContext.request.contextPath}/albumsController/createAlbum.html">
							...
							<div class="albumInfosBlock">
								<table>
									...
									<tr>
										...
										<td>
											<div id="selectContainer">
												<form:select path="selectedCoverAlbum" onchange='javascript:changeCoveringImage();' class='selectClass' id='selectedCoverAlbum'>
													<form:option value='' selected='selected'>-CHOISIR UNE COUVERTURE-</form:option>
													<c:forEach var="aSelectedPicture" items="${albumForm.selectedPictures}">
														<form:option value="${aSelectedPicture.pageIndex}/${aSelectedPicture.photoIndex}">Page=${aSelectedPicture.pageIndex}  - Photo=${aSelectedPicture.photoIndex+1}</form:option>
													</c:forEach>
											</form:select>
											</div>
										</td>
									...
									<tr>
										...
										<td><form:input autocomplete="off" path="name"  cssClass="inputClass"/></div></td>
									</tr>
									<tr>
										<td><span class="albumTitle">Mes thèmes : </span></td>
										<td><form:select path="selectedOwnShelf" cssClass="selectClass" disabled="${empty albumForm.userShelves}">
											<form:option value="" selected="selected">-CHOISIR UN THEME-</form:option>
											<c:forEach var="aShelf" items="${albumForm.userShelves}">
								...
								<div class="additionnalBlock"><span class="albumTitle">Description de l'album : </span>
									<div>
