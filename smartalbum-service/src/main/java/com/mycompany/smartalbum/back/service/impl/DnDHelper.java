...
public class DnDHelper {
	...
	public boolean processDrop(Object dragValue, Object dropValue) {
		...
		if(dragValue instanceof Image){
		...
		else if(dragValue instanceof ArrayList<?>){
			//If user drag album
			if(!((Album)dropValue).getOwner().getLogin().equals(user.getLogin())){
				//Drag in the shelf, that not belongs to user
				backService.getErrorHandler().addToErrors(Constants.ADD_ERROR_EVENT+"-"+Constants.DND_ALBUM_ERROR);
			...
			if( ((List<?>)dragValue).isEmpty())
			{
				return false;
			}
			
			return handleImages((List<Image>)dragValue, (Album)dropValue);
		}
		return false;
	}
	...
	private boolean handleAlbum(Album dragValue, Shelf dropValue) {
		...
		try{
			backService.getShelfDBService().editShelf(dropValue);
	...
	private boolean handleImages(List<Image> dragValues, Album dropValue) {
		
		Map<Long, String> imageListClonePaths = getImagesMap(dragValues);
		
		for(Object object : dragValues)
		{
			Image dragValue = (Image)object;
			if(dragValue.getAlbum().equals(dropValue)){
			    LOG.warn(Constants.ADD_ERROR_EVENT+"-"+"Tentative de déplacer une image dans l'album qui la contient");
				return false;
			}
		...
		for(Object object : dragValues)
		{
			Image dragNewValue = (Image)object;
			String pathOld = imageListClonePaths.get(dragNewValue.getId());
			backService.getFileSystemService().renameImageFile(dragNewValue, pathOld);
		}
		return true;
	}
	
	private Map<Long,String> getImagesMap(List<Image> imageListClone)
	{
		Map<Long,String> result = Maps.newLinkedHashMap();
		for(Image img : imageListClone)
		{
			result.put(img.getId(), img.getFullPath());
		}
		return result;
	}
